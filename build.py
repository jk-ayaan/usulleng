...
const state={region:"부산",edSel:[],q:"",catSel:[],guSel:[],sort:"def",view:"list",radius:0,loc:null};
...
function buildChips(elId,key,stateKey,label){
  const el=document.getElementById(elId);const sel=state[stateKey];
  let h=`<span class="chiplabel">${label}</span><button class="chip${sel.length===0?" on":""}" data-v="전체">전체<i class="ct">${rows().length}</i></button>`;
  tally(key).forEach(([v,n])=>h+=`<button class="chip${sel.indexOf(v)>=0?" on":""}" data-v="${esc(v)}">${esc(v)}<i class="ct">${n}</i></button>`);
  el.innerHTML=h;
  el.onclick=e=>{const b=e.target.closest(".chip");if(!b)return;const v=b.dataset.v;
    if(v==="전체")state[stateKey]=[];
    else{const i=sel.indexOf(v);if(i>=0)sel.splice(i,1);else sel.push(v)}
    buildChips(elId,key,stateKey,label);render()};
}
function buildEdChips(){
  ...
  const opts=[
    ...
    {v:"both",lab:"24·25",n:cnt(r=>r.ed.length===2)},
  ];
  ...
  el.onclick=e=>{const b=e.target.closest(".chip");if(!b)return;const v=b.dataset.v;
    ...
    buildEdChips();buildTabs();buildChips("cats","c","catSel","종류");buildChips("gus","g","guSel","지역");render();
    document.getElementById("subtitle").textContent=subtitle()};
}
function filtered(){
  const q=norm(state.q);
  let list=rows().filter(r=>{
    if(state.catSel.length&&state.catSel.indexOf(r.c)<0)return false;
    if(state.guSel.length&&state.guSel.indexOf(r.g)<0)return false;
...
function switchRegion(reg){state.region=reg;state.catSel=[];state.guSel=[];state.q="";document.getElementById("q").value="";
  document.getElementById("subtitle").textContent=subtitle();
  buildTabs();buildEdChips();buildChips("cats","c","catSel","종류");buildChips("gus","g","guSel","지역");
...
document.getElementById("subtitle").textContent=subtitle();
buildTabs();buildEdChips();renderDD();buildChips("cats","c","catSel","종류");buildChips("gus","g","guSel","지역");render();setToolsH();
</script>
</body>
</html>"""
